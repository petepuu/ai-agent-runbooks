...
flowchart TB
    subgraph USER["1. User and Event Layer"]
        ...
        W["{{REQUESTED_EVENT_ENTRY}}<br/>Workflows trigger"]
    end
    ...
    subgraph DATA["3. Data and Integration Layer"]
        ...
        D["Workflow integration<br/>Validate and act<br/>Exception handling"]
    end
    USER --> AGENT
    AGENT --> DATA
    U -->|"Request"| A
...
Keep three stacked layers and short labels. Explain responses in the sequences
instead of crowding this diagram with reverse edges. The event and delivery boxes
are stages of the same workflow, not agent tools. State setup status in prose,
