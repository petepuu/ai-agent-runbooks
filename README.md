...
- 🧑‍💼 [HR Onboarding Agent (GHCP)](./01-scenarios/HR-Onboarding-Agent-GHCP/1.Overview.md) - GitHub Copilot harness scenario with normal HR conversation, onboarding checklists and connected user-mailbox Workflows replies using two runtime skills. The workflow sends the agent's HTML through Outlook without per-message approval. This is a development walkthrough with documented limitations, not a deployed integration.
...
- 🧑‍💻 [Employee Self-Service (GHCP)](./01-scenarios/Employee-Self-Service-Agent-GHCP/1.Overview.md) — Draft Copilot Studio GitHub Copilot harness scenario with four runtime skills for read-only HR/IT guidance, request navigation and a proposed/OFF authenticated own IT request status extension.
...
- 🎟️ [Copilot Licence Lifecycle Agent (GHCP)](./01-scenarios/Copilot-License-Lifecycle-Agent-GHCP/1.Overview.md) — Draft GitHub Copilot harness design with ten runtime skills for inventory, usage, waitlist decisions, confirmed notifications, reclaim, assignment, cancellation and disputes. Backend contracts and tenant release gates are documented; integrations are not deployed.
