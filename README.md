...
- [HR Onboarding Agent](./01-scenarios/HR-Onboarding-Agent-GHCP/1.Overview.md) - GitHub Copilot harness scenario with all-employee HR guidance, onboarding checklists, draft-only chat and autonomous HR shared-mailbox Workflows replies without per-message approval, all included by default with four runtime skills. Operators configure and test dependencies during setup; documentation is not a deployed integration.
