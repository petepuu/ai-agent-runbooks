...
**Autonomous email quality and access:** approve a recipient-audience-safe corpus, or enforce recipient-specific filtering before retrieval and revalidation before send. Connection-owner or application access does not impersonate the employee. Use versioned whole-inquiry request rules for the permitted low-risk topics. The agent composes an HTML answer with a references table and agent sign-off. The workflow validates answer support, coverage, evidence and permitted HTML, then binds the sanitized body before sending. A source link or model confidence alone is not proof of correctness; unsupported or unverifiable answers are held. Missing evidence, unknown matching rules, medical/personal data, entitlement uncertainty or inaccessible sources hold for HR handling. This intentionally trades automatic coverage for an enforceable authorization boundary.
...
The [Outlook connector reference](https://learn.microsoft.com/en-us/connectors/office365/) documents **When a new email arrives** for the connected user's mailbox and email actions such as **Send an email** and **Reply to email (V3)** (`ReplyToV3`). The walkthrough uses the connected user mailbox with **Subject filter = HR question**, not a shared mailbox. These are documented primitives, not an implemented HR policy gate.

Use the mailbox belonging to the Outlook connection's signed-in user. Shared mailboxes are not part of the current setup. Validate the delegated account and send permissions; the connector documents no service-principal authentication. Choose the correct folder, set **Include Attachments = No**, and validate authoritative attachment metadata before allowing a reply. Do not trust From filters as sender authentication. Protected, oversized or invalid messages may be skipped; bursts may miss events and Dynamic Delivery can create duplicate events. The mailbox owner must retain normal mailbox coverage, and the integration owner must monitor missed/held events without promising every incoming mail is processed.
