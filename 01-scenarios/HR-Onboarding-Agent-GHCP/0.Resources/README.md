...
The local sample Word documents support the article-creation steps, not production policy or the entire acceptance suite. Review their contents before loading them into an isolated test knowledge base. The runbook retains 72 screenshot placements and the connector guide 30; the 102 local images are editing placeholders, not current-UI evidence. Superseded settings are labeled **Screenshot to replace**. The verification URL in `Images/012.png` is redacted because it contains account-verification material; replace it with a safely captured screenshot from your own setup, never an active token.
