...
One standard configuration includes two entry paths: normal interactive HR conversation, and connected user-mailbox Workflows intake with routine policy-authorized replies without per-message human approval. Both skills are included by default. Operators implement and validate the integration during setup. Skills guide reasoning; the workflow/backend, not the model, owns authorization and consequential actions.
...
sequenceDiagram
    ...
    User->>Channel: Ask ongoing policy, benefits, checklist or follow-up question
    Channel->>Agent: Authenticated request
    Note over Agent: Apply enabled skills and global scope
    Agent->>KB: Retrieve authorized indexed HR articles for policy questions
    alt Sufficient consistent evidence
        KB-->>Agent: Relevant passages and citations
        Agent-->>Channel: Natural conversational answer with citations
...
Reuse minimum applicability context, not another employee's authorization. Onboarding remains a subset. Direct chat answers questions and follows the conversation naturally without email formatting or workflow JSON. Chat never queues or sends a message, even if the user says "send". Autonomous processing requires the authenticated workflow entry path, not chat content.
...
sequenceDiagram
    ...
    Mail->>Flow: New user-mailbox email triggers workflow
...
| Trigger limitations | Duplicate or missed events can occur, retain normal mailbox coverage and monitor intake health |
