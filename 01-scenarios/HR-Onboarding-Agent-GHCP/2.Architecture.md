...
| Onboarding planning | `hr-onboarding-checklist` | Search configured knowledge, then return chat-only numbered phases using retrieved guidance and supplied details; no email composition or task writes |
...
The checklist skill also searches configured knowledge before answering, whether the user supplies new-hire details or asks a general onboarding question. It groups the guidance into numbered phases and uses placeholders for unknown specifics. The skill's sample tasks are not company policy; policy claims still require applicable evidence. Email replies use the separate email skill rather than the chat checklist.
