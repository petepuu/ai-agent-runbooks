...
flowchart TD
    ...
    A["HR Onboarding Agent GHCP<br/>Copilot Studio - GitHub Copilot harness<br/>Policy / checklist / draft / reply skills"]
    ...
    M["HR Shared Mailbox Event<br/>Authenticated event intake"]
    W["Copilot Studio Workflows<br/>Trusted intake and employee mapping"]
    G["Backend Validation<br/>Current policy and recipient-safe evidence<br/>Server-issued payload authorization"]
    E["Autonomous Reply<br/>Deterministic send and status<br/>No per-message approval"]
    H["HR Exception Queue<br/>Confirmed handoff required"]
    ...
    A -->|"Compose reply"| D
    M -->|"Outlook shared-mailbox trigger"| W
    W -->|"Agent node invokes published GHCP agent"| A
    A -->|"hr-email-reply result through workflow"| G
    G -->|"Allowed routine response"| E
    G -->|"Held or denied, no automatic reply"| H
    W -->|"Unsafe intake or invocation failure"| H
    E -->|"Failed or unresolved attempt, reconcile before retry"| H
