...
**Agent Type**: Knowledge-grounded agent with autonomous capability using a Workflows email trigger (gated target)<br>
**Primary Tools**: Microsoft Copilot Studio, Workflows, ServiceNow Knowledge Copilot Connector<br>
**Complexity**: Intermediate<br>
**Status**: 📝 Draft

This scenario describes an HR Onboarding Agent for **all employees**, throughout their employment: company policies, ongoing benefits and wellness, leave processes, workplace guidance and onboarding as one subset. It uses authoritative HR knowledge grounded in a ServiceNow Knowledge Base. The intended autonomous email capability responds to routine, in-scope inquiries received by the HR team **without per-message human intervention**, under a release-approved, server-enforced policy.

The supplied artifacts are documentation and runtime skill definitions, not a deployed agent. Chat is the baseline. The autonomous Workflows profile is in scope but **OFF pending implementation and tenant release gates**. Current Microsoft documentation describes a GHCP Workflows Agent node that calls an existing published agent; the exact HR GHCP target, execution identity and end-to-end invocation still require validation. See [invocation evidence](0.Resources/README.md#workflow-invocation-finding).
...
The **HR Onboarding Agent** uses approved ServiceNow HR knowledge and four independently scoped runtime skills. Its baseline produces cited answers for all employees, onboarding checklists and email reply text in chat. Its intended **autonomous-workflow** profile receives HR mailbox events, validates trusted intake, invokes the GHCP agent for a grounded reply candidate, validates that result, and sends through deterministic workflow/backend logic. It remains **OFF** until implemented, authorized and tested.

Agent-wide instructions guide scope and grounding; backend controls enforce consequential boundaries. Skills supply task-specific procedures, not connections or event subscriptions. Routine authorized replies do not wait for an HR approval click. Exceptions go to a restricted HR handling queue only when that proposed integration confirms receipt; the agent never invents a successful handoff.
...
| Autonomous email reply — gated / OFF | `hr-email-reply` prepares a structured grounded result for a calling workflow; backend policy authorization, recipient-safe evidence, exact payload binding and reconciliation govern sending, not an LLM eligibility flag |
...
```mermaid
flowchart TD
    U["All Employees / HR Team<br/>Chat question or supplied inquiry"]
    ...
    M["HR Shared Mailbox Event<br/>Proposed configured intake - OFF"]
    W["Copilot Studio Workflows - OFF<br/>Trusted intake and employee mapping"]
    G["Proposed Backend Validation - OFF<br/>Current policy and recipient-safe evidence<br/>Server-issued payload authorization"]
    E["Autonomous Reply - OFF<br/>Deterministic send and status<br/>No per-message approval"]
    H["HR Exception Queue - OFF<br/>Confirmed handoff required"]
    ...
    A -->|"Compose reply"| D
    M -.->|"Documented Outlook trigger, tenant gated"| W
    W -.->|"Agent node, exact GHCP invocation gate"| A
    A -.->|"hr-email-reply structured candidate"| G
    G -.->|"Allowed routine response"| E
    G -.->|"Held or denied, no automatic reply"| H
    W -.->|"Unsafe intake or invocation failure"| H
...
Solid paths show the intended baseline after source and tenant setup, not a deployed agent. Dashed paths are proposed and OFF. A pasted inquiry is not a trusted email event. Workflows invokes the agent, then validates and sends its result; this is distinct from adding a workflow as a tool called by the agent. No native Email channel or executable workflow is supplied.
...
| Reduce routine mailbox handling | Measure policy-authorized autonomous replies, exception rate and response latency; require zero unauthorized disclosures or duplicate replies in the gated pilot |
...
- An **OFF**, intended autonomous Workflows capability for routine low-risk employee inquiries, without per-message approval, with trusted intake, backend policy authorization, verified recipient access and duplicate-send prevention.
...
| HR-approved audience-safe response catalog | Versioned source-backed answer blocks and routine request rules for autonomous email; must be implemented and governed | Email policy / validation |
