...
12. In **Build → Model**, select an available approved generally available model and **Save**. Do not infer a model or provider permission from screenshots, and do not use experimental/preview models for production. Leave memory off initially. See [model selection](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/authoring-select-agent-model).
