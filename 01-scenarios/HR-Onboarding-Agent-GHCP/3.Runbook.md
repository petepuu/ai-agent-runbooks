...
| Teams Custom App Side-loading | Enable only for an approved pilot if the chosen distribution route requires it | Teams Admin Center |
...
> ![Integrated Apps page in the Microsoft 365 Admin center](0.Resources/Images/001.png)

4. In the panel, find **"Allow the following users access to Copilot agents"**
5. Verify your user is included in the **approved pilot group**. Expand access to the approved all-employee audience only after release sign-off; do not select All users merely for setup convenience.
> !["Allow the following users access to Copilot agents" settings panel](0.Resources/Images/002.png)
>
...
> ![Teams Admin center - Teams Apps - Manage apps](0.Resources/Images/003.png)
>
4. Under **Custom apps**, review **"Let users interact with custom apps in preview"** with the Teams admin. Change the organization setting only when approved and required for the pilot; do not enable tenant-wide preview access solely for this exercise.
> ![Org-wide app settings](0.Resources/Images/004.png)
>
5. Go to **Teams apps** → **Setup policies** → select or create a policy assigned to the **approved pilot group**, not Global (Org-wide default)
> ![Teams apps - Setup policies - Global Org-wide](0.Resources/Images/005.png)
>
6. If sideloading is required, toggle **"Upload custom apps"** → **On** in that scoped policy → **Save** and verify its assignment
> ![Upload custom apps - on](0.Resources/Images/006.png)
...
> **Purpose**: ServiceNow developer demo content does not supply approved HR policy. Add HR-specific test content for the agent walkthrough.
> Download the HR-specific [Contoso Employee Handbook](0.Resources/Sample-documents/Contoso%20Employee%20Handbook.docx) and [Wellness Benefits Sample Knowledge](0.Resources/Sample-documents/Wellness%20Benefits%20Sample%20Knowledge.docx).
> These are synthetic test documents, not approved company policy. Use an isolated test knowledge base; HR must approve authoritative content, applicability, versions and citations before production. Apply the [knowledge quality requirements](0.Resources/README.md#knowledge-quality-requirements).
>
...
> ![ServiceNow login screen](0.Resources/Images/039.png)

3. Click **"All"** in the navigation on the top
4. Navigate to **Knowledge** → **Administration** → **Knowledge Bases**

> ![ServiceNow navigation showing Knowledge > Administration > Knowledge Bases](0.Resources/Images/040.png)

5. Open the approved **isolated test knowledge base** (the screenshots use **IT**). For production use the HR-owned base with approved article and base permissions.

>![Knowledge Bases list with "IT" selected](0.Resources/Images/041.png)

6. **Verify the approved publication workflow**:
   - Click the magnifier icon next to **"Publish workflow"**
   - Keep the HR-approved review/approval workflow; do not switch to **Knowledge – Instant Publish**
   - If an authorized change is needed, select the approved workflow and click **Update**
> **Screenshot to replace:** select the approved publication workflow, not Instant Publish.
>![Workflow change dialog with "Knowledge – Instant Publish" selected](0.Resources/Images/042.png)
>![Update](0.Resources/Images/043.png)

7. Click **"All"** in the navigation on the top
8. Navigate to **Knowledge** → **Knowledge Center**

> **Screenshot to replace:** use your approved publication workflow and test base; the visible Instant Publish setting is not the required configuration.
>![IT Knowledge Base page showing the "New" button in the Knowledge section](0.Resources/Images/044.png)

9. Click on **"Create an article"** in the **"Actions"** section on the right side of the page
>![IT Create an article in the Actions section](0.Resources/Images/045.png)

10. In **"Select knowledge base"**, select your isolated test base; under **"Select the Article Template"**, click **"Standard"** → **"Next"**

>![Article Template Selector page with Standard selected](0.Resources/Images/046.png)
...
11. Double-click **"Text"** in the **"Component"** of the **"Blocks"** and fill in the following fields:
    ...
    | Category | **"Policies"** |
    | Article body | Paste content from [`Contoso Employee Handbook.docx`](0.Resources/Sample-documents/Contoso%20Employee%20Handbook.docx) → Choose **"Keep formatting"** when prompted |

12. Click **Submit**

>![Knowledge New Record page showing Short description, Article body, and Category fields](0.Resources/Images/047.png)

13. Find the newly created article (Status: "Draft") → Click the **article Number** link
14. Click **"Publish"** to start the configured approval/publication process; have the authorized reviewer approve it

>![Knowledge edit page with the "Publish" button highlighted](0.Resources/Images/048.png)

15. Click **"All"** in the navigation on the top
16. Navigate to **Knowledge** → **Administration** → **Knowledge Bases**
17. Confirm the article's **Workflow status** shows **"Published"** only after approval completes

>![Article showing "Published" status in the Workflow column](0.Resources/Images/049.png)

---

#### Article 2: Comprehensive Guide to Health and Wellness Benefits

Repeat **Steps 7–17** above with the following values:

| Field | Value |
|---|---|
| Short description | `Comprehensive Guide to Health and Wellness Benefits` |
| Category | `Policies` |
| Article body | Paste content from [`Wellness Benefits Sample Knowledge.docx`](0.Resources/Sample-documents/Wellness%20Benefits%20Sample%20Knowledge.docx)|

>![Completed Health and Wellness Benefits article in the Knowledge edit page](0.Resources/Images/050.png)
>![Publish the article](0.Resources/Images/051.png)

After completing both articles, return to your **test knowledge base** and verify that **two new articles** are visible in the Knowledge section.
>![Verify the result](0.Resources/Images/052.png)
...
>![M365 Admin Center, Copilot, Connectors, and Connections](0.Resources/Images/053.png)

3. Click your **ServiceNow Knowledge connector** (e.g., `ServiceNowKB5`)
4. Click **"Start full crawl"**

>![ServiceNow Connector panel with the "Start full crawl" button](0.Resources/Images/054.png)
...
6. Also verify actual article IDs, working citations, source currency and permitted/denied/revoked non-maker access after content and identity synchronization. Count alone is not proof of correct permissions. For workflow retrieval, prove an HR-approved audience-safe corpus or recipient-entitlement filtering **before retrieval**, then current entitlement again before send; the connection owner's rights do not grant the recipient access.
...
Follow [Create a new agent](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/build-new-agent) and [Agent settings](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/settings-overview) for the GitHub Copilot harness.

1. Go to **Copilot Studio** → https://copilotstudio.microsoft.com
2. Select the approved **Developer Environment (e.g., Copilot)** created in Phase 0 and verify that the GitHub Copilot agent experience is available

> **Screenshot to replace — GHCP:** environment selector and current authoring experience.
> ![Copilot Studio environment selection screen](0.Resources/Images/055.png)

3. Click **"Agents"** in the left navigation

> **Screenshot to replace — GHCP:** current Agents navigation.
> ![Copilot Studio left navigation with "Agents" selected](0.Resources/Images/056.png)

4. Click **"New agent"** (or use **Home → Agent**). The designer opens on **Build** with the name field in focus.

> **Screenshot to replace — GHCP:** use New agent, not the standard-harness blank-agent flow.
> ![Agents page with the "New agent" button](0.Resources/Images/057.png)

5. Enter the exact name. Use **… → Settings → Agent details** to review primary language, solution and schema name **before the first save**, when these settings become read-only. Paste the [complete global instruction block](0.Resources/Release-readiness.md#global-agent-instructions) into **Instructions**. Record the description below for the current designer/publish checks; confirm its actual entry field in G1 rather than assuming a Details → Edit dialog.

   | Field | Value |
   |---|---|
   | Name | `HR Onboarding Agent` |
   | Description | `HR policy guidance, onboarding checklists, chat email drafts and workflow reply preparation for all employees.` |
   | Primary language | Approved employee support language |
   | Solution | Approved HR solution in this environment |
   | Schema name | Review before saving; the last name entered before first save determines the generated schema name |

> **Screenshot to replace — GHCP:** name/description and Agent details settings; use HR Onboarding Agent.
> ![Edit name and description](0.Resources/Images/058.png)

> **Screenshot to replace — GHCP:** current identity fields and complete Instructions section.
> ![Update the name and description](0.Resources/Images/059.png)


6. Click the **Save** icon. In **… → Settings → Safety & access**, select **Authenticate with Microsoft** for internal employee access. Choose the approved moderation level in **AI & behavior**; leave **Allow other agents to connect** disabled unless explicitly required and approved. It is not evidence of inbound Workflows support.

> **Screenshot to replace — GHCP:** current Save and authenticated access settings.
> ![Save](0.Resources/Images/060.png)

7. In **Build → Model**, select an available approved generally available model and **Save**. Do not infer a model or provider permission from screenshots, and do not use experimental/preview models for production. Leave memory off initially. See [model selection](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/authoring-select-agent-model).
...
| Phase 0 | Teams pilot policy and sideloading requirement reviewed | ☐ |
