...
# Step-by-Step Guide — HR Onboarding Agent
...
2. Name it `HR Onboarding Agent`. Before the first save, inspect **Agent settings** for primary language, solution and schema name; the name before first save determines the schema name.
...
This selects the connector-backed knowledge already indexed in Microsoft 365. Runtime retrieval uses M365 search/semantic index, not a direct connection from the agent to ServiceNow. The connector configured in Phase 1 owns ingestion and permission synchronization.
