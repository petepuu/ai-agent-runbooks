...
> ![Publish the agent before adding it to a Workflow](0.Resources/Images/056.png?v=147761594e5d)
...
> ![Change the trigger type to Connector](0.Resources/Images/069.png?v=866f983b56b2)

4. Select **Office 365 Outlook**.

> ![Select the Office 365 Outlook connector](0.Resources/Images/070.png?v=46bfe700fbaa)
...
6. Set **Original Mailbox Address** to your own email address for testing (in production, use the actual shared mailbox provided by the HR team). From **Advanced parameters**, select **Subject filter**.

> ![Set Original Mailbox Address and select Subject filter](0.Resources/Images/071.png?v=88c0d20c3a58)

> **Mailbox compatibility:** The selected shared-mailbox V2 trigger (`SharedMailboxOnNewEmailV2`) requires a shared mailbox your connection can access. If your own address is an ordinary personal mailbox, use the personal-mailbox trigger for testing instead, or use an accessible test shared mailbox here. Entering an address does not establish trigger support or access.

7. Set **Subject filter** to **HR question**.

> ![Set the Subject filter value to HR question](0.Resources/Images/072.png?v=9a43c696e06b)

> Keep **Include Attachments = No**. Backend intake still checks authoritative attachment metadata; this switch does not reject attached mail. A subject filter is not an authorization control. Before invoking the agent, validate transport authenticity, employee mapping, stable message identity, rate/loop suppression and full request coverage through `AcceptHrEmailEvent`.

8. Click **+** after the trigger to add a new step.

> ![Click the plus button after the trigger to add a step](0.Resources/Images/073.png?v=d0b66a5cbca8)

9. Select **Agent**.

> ![Select Agent from the Add panel](0.Resources/Images/074.png?v=ab6f6a190055)

10. Select **An existing agent** → the published **HR Onboarding Agent**. Do not create an inline agent or substitute an unverified Execute Agent connector route. Complete the remaining instructions and skill setup, then republish before testing this node.
11. In **Message**, supply the sanitized inquiry and isolated trusted context and allowed topics from accepted intake. Record exact GHCP target/version, imported skill execution and effective retrieval identity. Keep **Request human assistance when unsure** disabled; exceptions use the fixed HR queue, not email to the connection owner.

12. In the **same workflow**, parse the returned candidate as untrusted data. Run `ValidateHrEmailReply`, then `SubmitHrEmailReply` **only** on an authorized server result, using its `authorizationId` and `validatedResultId`. Send only the validated, sanitized HTML body bound by the backend; never accept model-provided recipients or authorization.
13. Route held/failed/malformed results to `QueueHrEmailException`, and pending/unknown submissions to `GetHrEmailReplyStatus`. Inspect every branch, retry setting and version before saving. Routine authorized replies need no per-message human approval.

14. Keep send credentials physically absent or revoked during isolated testing. Workflow node tests can call real APIs; mock input alone is not isolation. Activate the approved real-mail pilot only after E1–E2/D3 pass and release/mailbox owners authorize it. Native Email is unavailable in the [GHCP channel table](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/publication-channels-overview); this workflow supplies the email entry path.
