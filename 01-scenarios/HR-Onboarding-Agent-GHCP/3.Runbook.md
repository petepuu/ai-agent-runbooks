...
### Step 2-3. Add Workflow Email Trigger

Use [Copilot Studio Workflows](https://learn.microsoft.com/en-us/microsoft-copilot-studio/workflows-experience/flows-overview) and its [Agent node](https://learn.microsoft.com/en-us/microsoft-copilot-studio/workflows-experience/agent-node-workflow), **not a direct standard-harness agent trigger**. Complete Steps 2-4 through 2-6 and isolated publishing in Step 3-2 before selecting the existing agent. Then return here. See [detailed workflow setup](0.Resources/Release-readiness.md#workflow-setup).
...
7. In the **same workflow**, parse the returned candidate as untrusted data. Run `ValidateHrEmailReply`, then `SubmitHrEmailReply` **only** on an authorized server result, using its `authorizationId` and `validatedResultId`. Send only the validated, sanitized HTML body bound by the backend; never accept model-provided recipients or authorization.
